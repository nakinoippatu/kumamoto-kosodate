...
def _fetch_center_pdf_urls(html: str) -> dict[str, str]:
    ...
    テーブルの各行に施設名が含まれるセルとPDFリンクが並んでいる構造を利用。
    最新おたより（当月分 = リスト先頭のPDF）を取得する。
    """
    soup = BeautifulSoup(html, "html.parser")
    result: dict[str, str] = {}
    ...
    for tr in soup.find_all("tr"):
        ...
        for cdef in CENTER_DEFS:
            ...
            if key not in cell_text:
                continue
            # tr内の最初のPDFリンクを取得
            for a in tr.find_all("a", href=re.compile(r"\.pdf", re.I)):
                href = a.get("href", "")
                url = href if href.startswith("http") else BASE_CITY + href
                result[key] = url
                break
